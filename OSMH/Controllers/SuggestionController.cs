...
namespace OSMH.Controllers
{
    public class SuggestionController : Controller
    {
        ...
        public virtual ActionResult Index(string sortOrder, string searchString)
        {
			...
			var suggestions = from s in db.Suggestions.Include("User").Include(s => s.SuggestionComments)
							  select s;
		...
		{
			int currentUserId = db.users.Where(u => u.UserName == User.Identity.Name).Single().Id;
			List<UserSuggestion> userSuggestions = new List<UserSuggestion>();
			ViewBag.TitleSortParm = String.IsNullOrEmpty(sortOrder) ? "title_desc" : "";
			ViewBag.DateSortParm = sortOrder == "Date" ? "date_desc" : "Date";
			ViewBag.VoteSortParm = sortOrder == "mostPopular" ? "leastPopular" : "mostPopular";
			var suggestions = from s in db.Suggestions.Include("User").Include(s => s.SuggestionComments).Include(s => s.SuggestionUpvotes).Where(s => s.UserId == currentUserId)
							  select s;
		...
		public ActionResult Create([Bind(Include = "UserId, Title, Description, GroupName")] Suggestion suggestion)
		{
			...
			suggestion.UserId = db.users.Where(u => u.UserName == User.Identity.Name).Single().Id;
		...
		protected UserSuggestion MakeAUserSuggestion(Suggestion suggestion, string flag="index")
		{
			...
			if (flag == "detail")
			{
				foreach (var comment in suggestion.SuggestionComments)
				{
					...
					suggestionCommentView.UserName = comment.User.UserName;
		...
		public JsonResult Upvote(int? id)
		{
			...
			upvote.UserName = User.Identity.Name;
		...
		public JsonResult DownVote(int? id, string userName)
		{
			...
			SuggestionUpvote upvote = db.SuggestionUpvotes.Where(s => s.SuggestionId == id).Where(s => s.UserName == userName).Single();
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public ActionResult CreateComment([Bind(Include = "UserId, SuggestionId, Description")] SuggestionComment comment)
		{
